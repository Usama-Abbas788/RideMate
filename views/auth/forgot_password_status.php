...
<main class="auth-page">
  <div class="auth-card animate-fade" style="max-width:460px;">
    <div class="auth-logo">
      ...
      <div class="tagline">Check your email</div>
    </div>
    ...
    <p style="color:rgba(255, 255, 255, 0.6); text-align:center; font-size:0.9rem;">
      Please check your inbox or spam folder. The link will expire in 15 minutes.
    </p>
    ...
    <form action="/ridemate/actions/forgot_password.php" method="POST" id="resend-reset-form">
      <input type="hidden" name="email" value="<?= htmlspecialchars($email) ?>" />
      <button type="submit" class="btn btn-primary w-100 btn-lg" id="resend-reset-btn" disabled>
        Resend Link
      </button>
    </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTimer() {
        if (remaining <= 0) {
            ...
            timerStatus.textContent = "You can now resend the link";
    ...
    document.getElementById('resend-reset-form').addEventListener('submit', function() {
        resendButton.disabled = true;
